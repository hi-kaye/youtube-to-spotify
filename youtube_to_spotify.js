...
async function getVideoInfo() {
  ...
  return Promise.all(youtube_data).then((results) => {
    return results
  })
  .catch((err) => {
    console.log("error")
...
async function collectUri(video_data) {
  ...
  return Promise.all(uri_array).then((results) => {
    return results
  })
  .catch((err) => {
...
function getSpotifyUri(song_name, artist) {
  ...
  return new Promise((resolve, reject) => {
    superagent
    .get(`https://api.spotify.com/v1/search?q=track%3A${track}+artist%3A${artist_no_space}&type=track&limit=1`)
    .set({
...
async function addSongToSpotifyPlaylist() {
  ...
  superagent
  .post(`https://api.spotify.com/v1/playlists/${playlist_id}/tracks?uris=${track_uris}`)
  .set({
      "Content-Type": "application/json",
      "Authorization": `Bearer ${spotify_token}`
  })
